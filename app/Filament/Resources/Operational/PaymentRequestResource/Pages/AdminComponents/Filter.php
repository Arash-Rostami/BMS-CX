...
trait Filter
{
    public static function filterByBankName(): SelectFilter
    {
        return SelectFilter::make('bank_name')
            ->label('Bank Name')
            ->options(function () {
                $filters = ['type' => 'distinct_bank_names'];
                return SmartCacheManager::remember('PaymentRequest', $filters, 720, function () {
    ...
    public static function filterByBeneficiary(): SelectFilter
    {
        return SelectFilter::make('payee_id')
            ->label('Recipient')
            ->options(function () {
                return SmartCacheManager::remember('Beneficiary', ['type' => 'select_options'], 720, function () {
                    return Beneficiary::pluck('name', 'id')->all();
                });
            })
            ->multiple()
            ->default(null)
    ...
    public static function filterByCaseNumber(): SelectFilter
    {
        return SelectFilter::make('case_number')
            ->label('Case No.')
            ->options(function () {
                $filters = ['type' => 'distinct_case_numbers'];
                return SmartCacheManager::remember('PaymentRequest', $filters, 720, function () {
                    return PaymentRequest::query()
                        ->distinct()
    ...
    public static function filterByContractor(): SelectFilter
    {
        return SelectFilter::make('contractor_id')
            ->label('Contractor')
            ->options(function () {
                return SmartCacheManager::remember('Contractor', ['type' => 'select_options'], 720, function () {
                    return Contractor::pluck('name', 'id')->all();
                });
            })
...
//            })
    ...
    public static function filterByReason(): SelectFilter
    {
        return SelectFilter::make('reason_for_payment')
            ->label('Reason')
            ->options(function () {
                return SmartCacheManager::remember('Allocation', ['type' => 'select_options'], 720, function () {
                    return Allocation::pluck('reason', 'id')->all();
                });
            })
            ->multiple()
...
//            })
    ...
    {
        return SelectFilter::make('monthly_data_pr')
            ->label('Period')
