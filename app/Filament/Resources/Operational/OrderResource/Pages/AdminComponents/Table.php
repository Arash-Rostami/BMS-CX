...
trait Table
{
    public static function showAllPayments(): TextColumn
    {
        return TextColumn::make('provisional_total')
            ...
            ->summarize(Summarizer::make()
                ->label('💰 Initial | Provisional | Final')
                ->using(fn(SummaryBuilder $query) => self::calculateSummaries('payment', $query))
                ->html()
                ->formatStateUsing(function (?string $state): ?string {
                    if (blank($state)) return null;
                    $parts = array_map(
                        fn(string $value): string => str_starts_with(trim($value), '-')
                            ? "<span style='color: orangered; font-size: small; letter-spacing: 0.5px;'>{$value}</span>" : $value,
                        explode(' ┆ ', $state)
                    );
     ...
     * @return TextColumn
     */
    public static function showVoyageNumberLegTwo(): TextColumn
    {
