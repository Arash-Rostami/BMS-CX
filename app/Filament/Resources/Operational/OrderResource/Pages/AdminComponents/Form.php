...
trait Form
{
    ...
    public static function getOrderStatus(): Select
    {
        return Select::make('order_status')
            ...
            ->options(self::$statusWithEmoji)
            ->disableOptionWhen(fn(string $value): bool => in_array($value, match (auth()->user()?->role) {
                'agent' => ['accounting_approved', 'accounting_rejected'],
                'accountant' => ['processing', 'closed', 'cancelled', 'accounting_review'],
                default => [],
            }));
    }
