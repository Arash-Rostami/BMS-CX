    ...
    public static function filterAccount(): SelectFilter
    {
        return SelectFilter::make('account_id')
            ...
            ->query(fn(Builder $query, array $data): Builder => $query
                ->when($data['value'], fn($q, $v) => $q->whereHas('ledgerEntry', fn($q) => $q->where('account_id', $v)))
            );
    }
    ...
    public static function filterTransactionDate(): FilamentFilter
    {
        return FilamentFilter::make('transaction_date')
            ...
            ->indicateUsing(function (array $data): array {
                $indicators = [];
                if ($data['created_from'] ?? null) $indicators['created_from'] = 'From ' . $data['created_from'];
                if ($data['created_until'] ?? null) $indicators['created_until'] = 'Until ' . $data['created_until'];
                return $indicators;
            });
    }
