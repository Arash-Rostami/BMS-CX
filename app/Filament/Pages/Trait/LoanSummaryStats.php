...
trait LoanSummaryStats
{
    protected function prepareLoanSummaryStats(Builder $query): array
    {
        ...
        $totals = Cache::remember($cacheKey, now()->addMinutes(15), fn() => [
            'total_disbursed'    => (clone $query)->sum('total_disbursed_base'),
            ...
            'total_settled'      => (clone $query)->where('is_settled', true)->count(),
            'total_unsettled'    => (clone $query)->where('is_settled', false)->where('entry_type', 'disbursement')->count(),
        ...
        $outstandingBorder       = $hasOutstanding ? 'light-dark(#fde68a, #d97706)' : 'light-dark(#bbf7d0, #16a34a)';
