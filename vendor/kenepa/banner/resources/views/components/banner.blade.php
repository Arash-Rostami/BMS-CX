...
>
    @if ($banner->isVisible())
        @php
            ...
            $plainContent = strip_tags($banner->content);

            $isFarsi = preg_match('/[\x{0600}-\x{06FF}]/u', $plainContent);
                ...
                <div>
                    @if ($banner->icon)
                        <x-filament::icon
                            ...
                            class="h-6 w-6 ml-2 mr-2 text-gray-500 dark:text-gray-400 text-white"
                        />
                    @endif
                </div>
                <div>
